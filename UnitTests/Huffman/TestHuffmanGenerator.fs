...
module FromBitCounts =
    ...
    let ``1 character with length (0) generates empty bit stream`` () =
        ...
        }, HuffmanGenerator.GenerateFromBitCounts([|
            HuffmanGenerator<_>.Entry('a', 0uy)
        |]).GenerateValueMap())

    [<Fact>]
    let ``2 characters with lengths (1, 1) generate correct bit stream`` () =
        Assert.Equal<IEnumerable<KeyValuePair<_, _>>>(seq {
    ...
    [<Fact>]
    let ``3 characters with lengths (2, 2, 1) generate correct bit stream`` () =
        Assert.Equal<IEnumerable<KeyValuePair<_, _>>>(seq {
    ...
    [<Fact>]
    let ``4 characters with lengths (2, 2, 2, 2) generate correct bit stream`` () =
        Assert.Equal<IEnumerable<KeyValuePair<_, _>>>(seq {
    ...
    [<Fact>]
    let ``4 characters with lengths (2, 1, 3, 3) generate correct bit streams`` () =
        Assert.Equal<IEnumerable<KeyValuePair<_, _>>>(seq {
